...
for i = 1:m
	...
	if(observedLandmarks(landmarkId)==false)
		...
		mu(landmarkId*2 + 2) = x + r*cos(phi+theta);
		mu(landmarkId*2 + 3) = y + r*sin(phi+theta);
	...
	delta_x = mu(landmarkId*2 + 2) - x;
	delta_y = mu(landmarkId*2 + 3) - y;
	delta = [delta_x, delta_y];
	
	q = delta*delta';
	q_sq = sqrt(q);
	bearing = atan(delta_y/delta_x) - theta;
	
	expectedZ((i-1)*2+1) = q_sq;
	expectedZ((i-1)*2+2) = bearing;

	% TODO: Compute the Jacobian Hi of the measurement function h for this observation
	Hi = zeros(2,21);
	Hi(1,1:3) = [-q_sq*delta_x , -q_sq*delta_y, 0];
	Hi(1,landmarkId*2+2) = q_sq*delta_x;
	Hi(1,landmarkId*2+3) = q_sq*delta_y;
	Hi(2,1:3) = [delta_y, -delta_x , -q];
	Hi(2,landmarkId*2+2) = -delta_y;
	Hi(2,landmarkId*2+3) = delta_x;
	Hi = Hi/q;
