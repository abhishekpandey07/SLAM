...
def plot_estimated_landmarks(mu,sigma,map,ids,prob):
    ax = plt.gca()
...
def plot_and_save_state(mu,sigma,map,landmarks,ids,i,prob = 0.95):
    # clear figure
    plt.clf()
    ...
    plot_estimated_landmarks(mu,sigma,map,ids,prob=prob)
    plt.xlabel('x',size=12)
    plt.ylabel('y',size=12)
    plt.title('Timestep : {}, Confidence : {:.2f}%'.format(i+1,prob*100))
    plt.legend()
    # save figure to file
    plt.savefig('plots/ukf_{:>03}'.format(i+1))
